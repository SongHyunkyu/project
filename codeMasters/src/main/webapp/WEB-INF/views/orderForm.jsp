	...
	<div class="order-container">
		<div class="order-details">
			...
			<div class="customer-info">
				...
				<form action="/project/order_c" id="pay" method="POST" id="form">
					<input type="hidden" id="orderTotalField" name="total">
					<input type="hidden" name="currentDate" id="currentDate">
					<c:forEach var="item" items="${cart}">
						<c:forEach begin="1" end="${item.amount}" varStatus="loop">
							<input type="hidden" name="book_number" id="book_number"value="${item.book_number}">
						<input type="hidden" name="cart_number" id="cart_number" value="${item.cart_number}">
						</c:forEach>
					</c:forEach>
...
console.log(getCurrentDate());
   ...
   let items = [
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            html += '          <img src="' + items[i].imgUrl + '" style="width: 80px; height: 120px;"></td>';
 ...
 /*        function submitOrder() {
        	  const cart = JSON.parse(sessionStorage.getItem('cart')); // 세션 스토리지에서 카트 정보를 가져옴
        	  ...
        	  document.getElementById('pay').appendChild(orderItemsInput);
